...
def wally_map_from_dict(d):
    m = pointer(wally_map())
    assert(wally_map_init_alloc(len(d.keys()), m) == WALLY_OK)
    for k,v in d.items():
...
class DescriptorTests(unittest.TestCase):
    ...
    def test_parse_miniscript(self):
        ...
        for miniscript, child_num, expected in [
            ...
            ret, written = wally_descriptor_parse_miniscript(miniscript, keys, child_num, 0, script, script_len)
    ...
    def test_descriptor_to_scriptpubkey(self):
        ...
        for descriptor, child_num, network, expected in [
            ...
            self.assertEqual((ret, written), (WALLY_OK, SCRIPTPUBKEY_P2WPKH_LEN))
            self.assertEqual(script[:written], make_cbuffer(expected)[0])
